...
The `DEFAULT_ADMIN_ROLE` is the administrative role of the Diamond PAU's access-control contracts. It is authorized to grant and revoke all other roles of the Diamond PAU on the AccessControls contract. It is also authorized to set rate limits on the ALM Rate Limits contract. The Default Admin Role is held on both contracts by the Grove Proxy and by the Configurator, as specified in [A.2.2.10.1.1.1.2.3.6 - Configurator](5e1f82c7-bcd6-46f8-aec0-3e767e55a93c).
